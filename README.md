...
- Caches identical requests to avoid redundant model calls (see [caveats below](#response-caching) — this is a concept demo, not a production caching strategy)
...
The gateway includes a simple DynamoDB-backed cache: requests are hashed on `modelId + prompt`, and identical requests return the cached response instead of calling `Converse` again.
 
**Being honest about what this actually is:** exact-match caching on the raw prompt text is a weak strategy for real chat traffic — users rarely phrase the same question identically twice ("What's the capital of France?" vs. "capital of france?" vs. "france's capital city"), so the real-world hit rate would be close to zero. This is included as a **concept demonstration** of the caching pattern (cache key design, TTL, best-effort read/write that never blocks the actual response), not as a production-ready optimization.
 
**Approaches that would actually help in production:**
 
- **Semantic caching** — embed the prompt and compare against cached embeddings by similarity (e.g. cosine similarity above a threshold) instead of exact string matching, so paraphrased questions still hit the cache. Tools like [GPTCache](https://github.com/zilliztech/GPTCache) implement this pattern.
- **Provider-level prompt caching** — Bedrock and the Anthropic API support caching the *static* part of a prompt (system prompt, few-shot examples, long context) server-side, independent of whether the user's actual question repeats. This is what production LLM gateways typically rely on instead of full-response caching.
- **Caching by intent/entity instead of raw text** — e.g. for a "summarize this document" feature, keying the cache on document ID + operation rather than the prompt string (which might contain timestamps, session IDs, etc.) captures real repetition far more reliably.
- **KV-cache reuse at the inference layer** — if self-hosting (as this project does with Ollama), inference servers like vLLM can reuse the KV cache for shared prompt prefixes (e.g. a common system prompt across many requests), which is a different — and often more effective — optimization layer than gateway-level response caching.
Where exact-match caching *does* make sense: FAQ bots with a small fixed set of standard questions, batch/eval workloads with `temperature=0` and identical prompts, or — as here — local testing and demonstrating the pattern.
...
- **Response streaming** — deliberately out of scope; see [`docs/streaming-design-sketch.md`](docs/streaming-design-sketch.md) for why (short version: it would bypass the Phase 3 Lambda authorizer) and how it would be built.
## Lessons learned
